...
Main:
	...
	# Desvios Condicionais (beq) + Intruções R (add, sub, slt) = 15 + 4 + 4 + 4 = 27 pontos
	IF1:
	...
	# Desvios Incondicionais (j) + Aritméticas com Imediato (addi) = 10 + 5 = 15 pontos
	WHILE:
		slt $t0, $s4, $s1	# $t0 = i < b
 	...
 	# Operações de Memória (lw, sw) = 15 + 15 = 30 pontos
 	MEMORIA:
 		addi $s5, $0, 20	# $s5 = -4 
 		sw $s0, 0($s5)		# mem[2] = a
 		lw $s1, 0($s5)		# b = mem[2]
